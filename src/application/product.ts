...
*/
  ...
  try {
    ...
    return;// if we have return res.status(204).send(); in this format we need asynchandlers for the api routes ex;- get(asynchandler(getproducts)), so we change the format to return after definig the response
 } catch (error) {
    next(error);
  }
};
